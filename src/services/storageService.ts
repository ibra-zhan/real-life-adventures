...
export class StorageService {
  ...
  private async ensureUploadDirectory(): Promise<void> {
    try {
      ...
      for (const category of categories) {
        await fs.mkdir(path.join(this.config.basePath, category as string), { recursive: true });
      }
    } catch (error) {
    ...
    for (const file of mediaFiles) {
      try {
        ...
        if (file.compressedUrl) {
          const compressedKey = this.extractKeyFromUrl(file.compressedUrl);
          if (compressedKey) {
  ...
  async getStorageStats(): Promise<{
    ...
    Object.values(MediaCategory).forEach(category => {
      stats.byCategory[category as MediaCategory] = { count: 0, size: 0 };
    });
